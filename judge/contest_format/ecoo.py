...
class ECOOContestFormat(DefaultContestFormat):
    ...
    def display_user_problem(self, participation, contest_problem):
        ...
        if format_data:
            ...
            return format_html(
                '<td class="{state} ecoo_format"><a href="{url}">{points}{bonus}<div class="solving-time">{time}</div></a></td>',
                ...
                url=reverse('contest_user_submissions',
                            args=[self.contest.key, participation.user.username, contest_problem.problem.code]),
                points=floatformat(format_data['points']),
                bonus=bonus,
                time=nice_repr(timedelta(seconds=format_data['time']), 'noday'),
            )
        else:
            return mark_safe('<td class="ecoo"></td>')
    ...
    def display_participation_result(self, participation):
        return format_html(
            ...
            url=reverse('contest_all_user_submissions',
                        args=[self.contest.key, participation.user.username]),
