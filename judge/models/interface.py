...
class NavigationBar(MPTTModel):
    ...
    is_root = models.BooleanField(default=False, verbose_name=_('Root node'))
...
class BlogPost(models.Model):
    ...
    def is_editable_by(self, user):
        ...
        if user.has_perm('judge.edit_all_post'):
            return True
        return user.has_perm('judge.change_blogpost') and self.authors.filter(id=user.id).exists()
