...
class SubmissionAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        queryset = Submission.objects.select_related('problem', 'user', 'language').only(
            'problem__code', 'problem__name', 'user__username', 'language__name',
    ...
    def has_change_permission(self, request, obj=None):
        ...
        if request.user.has_perm('judge.edit_all_problem') or obj is None:
            return True
        return obj.problem.is_editor(request.user)
    ...
    def judge_view(self, request, id):
        ...
        if not request.user.has_perm('judge.edit_all_problem') and \
                not submission.problem.is_editor(request.user):
            raise PermissionDenied()
