...
def get_user_info(usernames):
    return {name: (rank, rating) for name, rank, rating in
            Profile.objects.filter(username__in=usernames)
                   .values_list('username', 'display_rank', 'rating')}
...
def link_user(user, noname=False):
    if isinstance(user, Profile):
        pass
    elif type(user).__name__ == 'ContestRankingProfile':
        user = user.user
    else:
        raise ValueError('Expected profile, got %s' % (type(user),))
    return {'user': user, 'noname': noname}
