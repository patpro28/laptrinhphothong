...
def api_v1_problem_info(request, problem):
    ...
    return JsonResponse({
        'name': p.name,
        'authors': list(p.authors.values_list('username', flat=True)),
...
def api_v1_user_list(request):
    queryset = Profile.objects.filter(is_unlisted=False).values_list('username', 'points', 'performance_points',
                                                                     'display_rank')
...
def api_v1_user_ratings(request, page):
    queryset = Profile.objects.filter(is_unlisted=False, is_active=True).values_list('username', 'rating')
