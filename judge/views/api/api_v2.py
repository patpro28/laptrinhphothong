...
class ProfileSimpleFilter(BaseSimpleFilter):
    def get_object(self, key):
        return Profile.objects.get(username=key)
        ...
        if not self.request.user.has_perm('judge.see_private_contest'):
            ...
            if self.request.user.is_authenticated:
                if self.request.user.has_perm('judge.edit_own_contest'):
                    q |= Q(authors=self.request.user)
                    q |= Q(curators=self.request.user)
                q |= Q(view_contest_scoreboard=self.request.user)
...
class APIProblemDetail(APIDetailView):
    ...
    def get_object_data(self, problem):
        return {
            'code': problem.code,
            'name': problem.name,
            'authors': list(problem.authors.values_list('username', flat=True)),
...
class APIUserList(APIListView):
    ...
    def get_unfiltered_queryset(self):
        ...
        return (
            Profile.objects
            .filter(is_unlisted=False, is_active=True)
            .annotate(
                username=F('username'),
...
class APISubmissionList(APIListView):
    ...
    def get_unfiltered_queryset(self):
        ...
        return (
            queryset
            .select_related('problem', 'user', 'language')
            .order_by('id')
            .only(
                'id',
                'problem__code',
