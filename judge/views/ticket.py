...
class NewTicketView(LoginRequiredMixin, SingleObjectFormView):
    ...
    def form_valid(self, form):
        ticket = Ticket(user=self.request.user, title=form.cleaned_data['title'])
...
class TicketView(TitleMixin, TicketMixin, SingleObjectFormView):
    ...
    def form_valid(self, form):
        message = TicketMessage(user=self.request.user,
    ...
    def get_context_data(self, **kwargs):
        ...
        context['ticket_messages'] = self.object.messages.select_related('user')
...
class TicketList(LoginRequiredMixin, ListView):
    ...
    def _get_queryset(self):
        return Ticket.objects.select_related('user').order_by('-id')
    ...
    def get_queryset(self):
        ...
        if self.filter_assignees:
            queryset = queryset.filter(assignees__username__in=self.filter_assignees)
        if self.filter_users:
            queryset = queryset.filter(username__in=self.filter_users)
    ...
    def get_context_data(self, **kwargs):
        ...
        context['filter_status'] = {
            ...
            'user_id': json.dumps(list(Profile.objects.filter(username__in=self.filter_users)
                                       .values_list('id', flat=True))),
            'assignee_id': json.dumps(list(Profile.objects.filter(username__in=self.filter_assignees)
                                           .values_list('id', flat=True))),
        ...
        return JsonResponse({
            ...
            'notification': {
                ...
                'body': '%s\n%s' % (_('#%(id)d, assigned to: %(users)s') % {
                    'id': ticket.id,
                    'users': (_(', ').join(ticket.assignees.values_list('username', flat=True)) or _('no one')),
