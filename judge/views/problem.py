...
class ProblemDetail(ProblemMixin, SolvedProblemMixin, CommentedDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['has_submissions'] = authed and Submission.objects.filter(user=user,
                                                                          problem=self.object).exists()
        contest_problem = (None if not authed or user.current_contest is None else
                           get_contest_problem(self.object, user))
        ...
        if contest_problem:
            ...
            if contest_problem.max_submissions:
                context['submissions_left'] = max(contest_problem.max_submissions -
                                                  get_contest_submission_count(self.object, user,
                                                                               user.current_contest.virtual), 0)
            num_solution = self.contest.limit_solution
            num_solution -= PublicSolution.objects.filter(author=user, problem=self.object, contest=self.contest).count()
        ...
        if user.is_authenticated:
            ...
            if not can_edit:
                tickets = tickets.filter(own_ticket_filter(user.id))
...
class ProblemSubmit(LoginRequiredMixin, ProblemMixin, TitleMixin, SingleObjectFormView):
    ...
    def contest_problem(self):
        if self.request.user.current_contest is None:
            return None
        return get_contest_problem(self.object, self.request.user)
    ...
    def remaining_submission_count(self):
        ...
        return max(
            ...
            max_subs - get_contest_submission_count(
                self.object, self.request.user, self.request.user.current_contest.virtual,
            ),
        )
    ...
    def get_form_kwargs(self):
        ...
        kwargs['instance'] = Submission(user=self.request.user, problem=self.object)
    ...
    def get_form(self, form_class=None):
        ...
        form.fields['source'].widget.theme = self.request.user.ace_theme
    ...
    def form_valid(self, form):
        ...
        if not self.request.user.is_superuser and self.object.banned_users.filter(id=self.request.user.id).exists():
        ...
        with transaction.atomic():
            ...
            if contest_problem is not None:
                ...
                self.new_submission.contest_object = self.request.user.current_contest.contest
                if self.request.user.current_contest.live:
            ...
            if submission_file is not None:
                ...
                source_url = submission_uploader(
                    ...
                    user_id=self.new_submission.user.id,
                )
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if submission_id is not None:
            ...
            if not request.user.has_perm('judge.resubmit_other') and self.old_submission.user != request.user:
                raise PermissionDenied()
        else:
...
class ProblemClone(ProblemMixin, PermissionRequiredMixin, TitleMixin, SingleObjectFormView):
    ...
    def form_valid(self, form):
        ...
        with revisions.create_revision(atomic=True):
            problem.save()
            problem.authors.add(self.request.user)
...
class PublicSolutionDetailView(TitleMixin, LoginRequiredMixin, SolutionMixin, CommentedDetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        if SolutionVote.objects.filter(voter=self.request.user, solution=self.get_object()).exists():
            context['vote'] = SolutionVote.objects.filter(voter=self.request.user, solution=self.get_object()).first().score
        else:
            context['vote'] = 0
        return context
...
def vote_solution(request, delta):
    ...
    vote: SolutionVote = SolutionVote.objects.get_or_create(voter=request.user, solution_id=solution_id)[0]
