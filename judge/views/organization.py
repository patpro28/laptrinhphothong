...
class OrganizationList(TitleMixin, LoginRequiredMixin, ListView):
    model = Organization
...
class OrganizationHome(LoginRequiredMixin, OrganizationDetailView):
...
class OrganizationUsers(LoginRequiredMixin, OrganizationDetailView):
...
class OrganizationRequestBaseView(LoginRequiredMixin, SingleObjectTemplateResponseMixin, SingleObjectMixin, View):
    ...
    def get_object(self, queryset=None):
        ...
        if not organization.admins.filter(id=self.request.user.id).exists():
            raise PermissionDenied()
        return organization
