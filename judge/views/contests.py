...
class ContestList(QueryStringSortMixin, DiggPaginatorMixin, TitleMixin, ContestListMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            for participation in ContestParticipation.objects.filter(virtual=0, user=self.request.user,
...
class ContestMixin(object):
    ...
    def is_tester(self):
        if not self.request.user.is_authenticated:
            return False
        return self.request.user.id in self.object.tester_ids
...
class ContestClone(ContestMixin, PermissionRequiredMixin, TitleMixin, SingleObjectFormView):
    ...
    def form_valid(self, form):
        ...
        with revisions.create_revision(atomic=True):
            ...
            contest.authors.add(self.request.user)
...
def base_contest_ranking_list(contest, problems, queryset):
    return [make_contest_ranking_profile(contest, participation, problems) for participation in
            queryset.select_related('user', 'rating').defer('user__about', 'user__organizations__about')]
...
class ContestParticipationList(LoginRequiredMixin, ContestRankingBase):
    ...
    def get_ranking_list(self):
        if not self.object.can_see_full_scoreboard(self.request.user) and self.profile != self.request.user:
            raise Http404()
...
def exportcsv(request, contest):
    ...
    for user in users:
        index += 1
        row = [index, user.user.name, user.user.username, user.score]
        writer.writerow(row)
