...
def _get_user_queryset(term):
    qs = Profile.objects
    if term.endswith(' '):
        qs = qs.filter(username=term.strip())
    else:
        qs = qs.filter(username__icontains=term)
    return qs
...
class UserSelect2View(Select2View):
    def get_queryset(self):
        return _get_user_queryset(self.term).annotate(username=F('username')).only('id')
...
class UserSearchSelect2View(BaseListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        self.object_list = self.get_queryset().values_list('pk', 'username', 'email', 'display_rank')
